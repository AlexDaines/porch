...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Text("Settings").font(PorchTheme.title).accessibilityAddTraits(.isHeader).accessibilityIdentifier("settings-title")
                Spacer()
            ...
            ScrollView {
                VStack(alignment:.leading,spacing:8) {
                    ...
                    DisclosureGroup {
                        ColorChoices(selection: $colorSelection, showsName: false).padding(.top, 4)
                    } label: {
                        HStack {
                            Text("Color")
                            Spacer()
