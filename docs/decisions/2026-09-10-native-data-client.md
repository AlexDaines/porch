...
The initial integration was read-only and local. The UAT revision adds explicit text sending within accepted conversations, with a send-attempt journal and no automatic retries. Instagram's HTML is loaded only for login/challenges. Feed, Stories and Messages have independent views and deliberate entry. There is no automatic story advancement, feed pagination or video playback. Unofficial endpoint changes remain a compatibility risk, concentrated in the small adapter.
