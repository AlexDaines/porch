...
Diagnostics are always enabled locally in Debug and Release. A bounded, protected JSONL timeline survives restarts and records app/build, lifecycle, network state, authentication transitions, serialized request stages/timing, fixed response shapes, filtering counts, native media failures and DM preparation, refusal, acknowledgement and reconciliation. Request UUIDs and installation-keyed HMAC references connect the events without exposing account IDs or message context. Unknown error envelopes are fingerprinted inside the isolated adapter; their text does not enter native diagnostics.
